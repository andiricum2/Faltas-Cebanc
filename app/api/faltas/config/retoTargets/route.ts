...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const filePath = `${baseDir}/retoTargets.json`;
    await writeJsonFile(filePath, retoTargets);
...
export async function GET(req: NextRequest) {
  ...
  try {
    const baseDir = getUserDataDir(dni);
    const filePath = `${baseDir}/retoTargets.json`;
    const retoTargets = await readJsonFile<Record<string, Record<string, boolean>>>(filePath);
